...
export function buildQuizPrompt(records) {
  ...
  return [
    ...
    "The clue must contain enough specific information to identify the entry from the four choices. Never submit a template-only clue such as 'This story', 'This person', 'This book', or 'This entry'. If a context is short, use the strongest complete detail from it and do not pad it with generic language.",
    "Do not repeat target_entry or any distinctive word, proper name, place name, number, or named phrase from target_entry in the clue. The answer choices are labels only; do not use the answer label as a clue. Rephrase the supplied context so the question tests the underlying event, premise, or reaction rather than recognition of the headline.",
